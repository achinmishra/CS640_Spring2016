	...
	public void handlePacket(Ethernet etherPacket, Iface inIface)
	{
		...
		if(getEntry == null || timeSpent > 15000)
		{
			...
			for(int i = 0; i < list.size(); i++)
			{
				...
				if(!temp.equals(inIface))
					sendPacket(etherPacket, temp); //broadcast to all the entries in the hashmap except to the one from where you received the packet from 
			}	
		}
		else
		{
