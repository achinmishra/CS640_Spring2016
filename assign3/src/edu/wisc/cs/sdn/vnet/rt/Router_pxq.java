...
{	
	...
	class RIPEntries
	{
		...
		int nextHop; 
                ...
                public RIPEntries(int cost, long timeStamp, int nextHop) {
                        this.cost = cost;
		...
		switch(etherPacket.getEtherType())
		{
			case Ethernet.TYPE_IPv4:
				...
				if(ip.getDestinationAddress() == ripBroadcastAddr && ip.getProtocol() == IPv4.PROTOCOL_UDP)
				{
					...
					else return ; //drop the UDP packet if its destination is not 520 or redirect if the request was not send at 520
	...
	public void initializeRIP() {
		for (Iface intFace : interfaces.values()) {
			...
			ripMappedEntries.put(getHashKey(netAdr, mask),new RIPEntries(0, -1, 0));
...
//                        boolean addEntry = !(ripMappedEntries.containsKey(key) && cost >= ripMappedEntries.get(key).cost);
			...
			if ( !(ripMappedEntries.containsKey(key) && cost >= ripMappedEntries.get(key).cost))
				addEntry = true; 
			if ( ripMappedEntries.containsKey(key) && ripMappedEntries.get(key).nexthop == ipAddr)
				addEntry = true; 	
                        ...
                        if (addEntry)
			{
				...
				ripMappedEntries.put(key, new RIPEntries(cost, System.currentTimeMillis(),ipAddr));
                                tableChanged = true;
                        }
                }
                if (tableChanged)
