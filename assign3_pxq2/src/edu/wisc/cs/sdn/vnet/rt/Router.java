...
{	
	...
	public void initializeRIP() {
	 ...
	 private void manageRip(Ethernet etherPacket, Iface interFace)
	 {
		...
		int routerIP = ipHead.getSourceAddress(); 

                if (ripHead.getCommand() == RIPv2.COMMAND_RESPONSE)
		{
                        manageRipResponse(routerIP, ripHead, interFace);
                } 
	 ...
	 private void manageRipResponse(int routerIP, RIPv2 ripHead, Iface interFace)
	 {
		 ...
		 for (RIPv2Entry ripEntry : ripHead.getEntries())
		 {
			 ...
			 if (!(ripMappedEntries.containsKey(key) && cost >= ripMappedEntries.get(key).cost))
				 addEntry = true; 


			 RouteEntry entry = routeTable.find(netAddr, subMask); 

			 if (entry != null && entry.getGatewayAddress() == routerIP) {
				 ripMappedEntries.get(key).cost = cost; 
				 if (cost >= 16) {
	 ...
	 {
		 //check link up/down 
		 //handle link come back
		 for (Iface iFace : interfaces.values()){
			 int mask = iFace.getSubnetMask();
			 int netAdr = getNetworkAddress(iFace.getIpAddress(), mask);
			 if (this.routeTable.find(netAdr, mask) == null) {
				 routeTable.insert(netAdr, 0, mask, iFace);
				 ripMappedEntries.put(getHashKey(netAdr, mask),new RIPEntries(0, -1));
                System.out.println("Line number: " + Thread.currentThread().getStackTrace()[1].getLineNumber() + " Method: " + Thread.currentThread().getStackTrace()[1].getMethodName());

			 }
		 }
		 //handle link goes down
		 List<RouteEntry> currEntries = new LinkedList<RouteEntry>(); //to hold entries with no gateway 
		 List<RouteEntry> tempEntries = new LinkedList<RouteEntry>();
		 for (RouteEntry entry : routeTable.entries){
		 ...
		 synchronized (currEntries){	
			 if (currEntries != null){
				 for (RouteEntry entry : currEntries){
					 for (Iface iFace : interfaces.values()){
						 int mask = iFace.getSubnetMask();
						 int netAdr = getNetworkAddress(iFace.getIpAddress(), mask);
						 if (entry.getDestinationAddress() == netAdr && entry.getMaskAddress() == mask){
							 tempEntries.remove(entry); 
						 }
					 }
				 }
				 if (tempEntries != null){
					 for (RouteEntry entry : tempEntries){
						 int netAdr = entry.getDestinationAddress(); 
						 int mask = entry.getMaskAddress(); 
						 routeTable.remove(netAdr, mask); 
						 ripMappedEntries.put(getHashKey(netAdr, mask),new RIPEntries(16, System.currentTimeMillis()));
                System.out.println("Line number: " + Thread.currentThread().getStackTrace()[1].getLineNumber() + " Method: " + Thread.currentThread().getStackTrace()[1].getMethodName());
...
System.out.println(routeTable.entries.toString()); 
