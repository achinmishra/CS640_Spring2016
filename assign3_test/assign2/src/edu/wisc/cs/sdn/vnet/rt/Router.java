...
{	
	...
	/**
	 * @return routing table for the router through dynamic RIP 
	 */ 
...
System.out.println("preparing ICMP msg");     	       	
    	//loop up the destination MAC address
    	//Get source IP header from the incoming package
    	int dstAddr = ipPacket.getSourceAddress(); 

System.out.println("icmp dst ip is " + dstAddr);
System.out.println("icmp routing talbe is " + this.routeTable);
    	//Find matching route table entry
    	RouteEntry bestMatch = this.routeTable.lookup(dstAddr); 
    	// if no entry matched, do nothing (which is not likely)
    	if (null == bestMatch) {
		System.out.println("icmp bestMatch null"); 
    		return; 
	}
    	...
    	ArpEntry arpEntry = this.arpCache.lookup(nextHop); 
    	if (null == arpEntry) {
		System.out.println("icmp arpentry null"); 
    		return; 
	}
...
System.out.println("finish preparetion");     	
